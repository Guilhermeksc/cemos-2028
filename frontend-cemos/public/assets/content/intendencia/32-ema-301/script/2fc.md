Sua tarefa é ler a lista de perguntas e respostas e verificar os arquivos cap2.md  para criar um banco de flash-cards no arquivo fc2.md, do seguinte modo:

    Criar exatamente 3 flashcards por página identificada no arquivo.

Formato da saída

O resultado final deve ser salvo exclusivamente em formato Markdown, no padrão de tabela abaixo.

Todas as colunas devem ser preenchidas, mesmo que com string vazia ("").

Instruções específicas:

    bibliografia_id": 67.

    pergunta será preenchido com a pergunta contextualizada.

    resposta: resposta curta, direta
    
    justificativa: justificativa curta, direta e necessariamente acompanhada da referência explícita(ex.: “Conforme o art. 2º, inciso I...” ou “Nos termos do item 4.3.1...”).

    prova deve ser branco.

    “paginas” será preenchido com o markador ## referente a página ao qual se refere, ex: se ## Página 11 então 'Pág 11 ...'
    
“assunto”: 241.

Exemplo de saída esperada:

| bibliografia_id | pergunta | resposta | prova | páginas | assunto | justificativa | caveira |
| 67 | Pergunta contextualizada | Resposta objetiva |  | Pág. 11 | Capítulo 2 - Conceitos Doutrinários Estratégicos | Conforme o item X.X do texto. |  |


Deverá salvar o resultado em formato markdown
