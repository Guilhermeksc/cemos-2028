...
from .serializers import (
    ...
    MateriaSerializer,
    CapitulosBibliografiaSerializer
)


class MateriaViewSet(viewsets.ModelViewSet):
    """ViewSet para CRUD completo de matérias."""

    queryset = MateriaModel.objects.all().order_by('materia')
    serializer_class = MateriaSerializer
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]
    search_fields = ['materia']
    ordering_fields = ['id', 'materia']
    ordering = ['materia']


class BibliografiaViewSet(viewsets.ModelViewSet):
    """ViewSet para gerenciar bibliografias e consultar capítulos relacionados."""

    queryset = BibliografiaModel.objects.select_related('materia').all()
    ...
    def capitulos(self, request, pk=None):
        """Lista os capítulos vinculados a uma bibliografia específica."""

        bibliografia = self.get_object()
        capitulos = CapitulosBibliografiaModel.objects.filter(bibliografia=bibliografia).order_by('id')
        serializer = CapitulosBibliografiaSerializer(capitulos, many=True)
        return Response(serializer.data)


class CapitulosBibliografiaViewSet(viewsets.ModelViewSet):
    """ViewSet para gerenciar capítulos das bibliografias."""

    queryset = CapitulosBibliografiaModel.objects.select_related('bibliografia').all()
    serializer_class = CapitulosBibliografiaSerializer
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['bibliografia']
    search_fields = ['capitulo_titulo', 'bibliografia__titulo']
    ordering_fields = ['id', 'capitulo_titulo', 'bibliografia__titulo']
    ordering = ['id']
...
# class MateriaViewSet(viewsets.ViewSet):
#     """ViewSet para listar matérias disponíveis"""
    
#     def list(self, request):
#         """Retorna lista de matérias com seus IDs"""
#         materias = MateriaModel.objects.all().order_by('materia')
#         data = [{'id': m.id, 'nome': m.materia} for m in materias]
